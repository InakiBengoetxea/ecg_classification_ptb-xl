...
def train_epoch(
    ...
    for batch in loader:
        ...
        # Dynamically handle multimodal vs single-modality inputs
        if len(batch) == 3:
            x_ecg, x_meta, y = batch
            x_ecg, x_meta, y = x_ecg.to(device), x_meta.to(device), y.to(device)
            preds = model(x_ecg, x_meta)
        else:
            x_ecg, y = batch
...
def get_predictions(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # Safe CPU translation guard for tensors arriving from GPU context paths
